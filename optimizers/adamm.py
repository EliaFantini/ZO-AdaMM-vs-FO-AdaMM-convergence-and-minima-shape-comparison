...
class AdaMM(Optimizer):
    ...
    def step(self):
        ...
        for group in self.param_groups:
            # TODO : keep entire sequences to be able to study the behavior
            ...
            for p in group['params']:
                ...
                if p.grad is not None:
                    ...
                    if len(state) == 0:
                        ...
                        # p is a tensor
                        state['gradient_avg'] = torch.zeros_like(p, memory_format=torch.preserve_format)
                        state['gradient_second_moment'] = torch.zeros_like(p, memory_format=torch.preserve_format)
                        state['gradient_max_second_moment'] = torch.zeros_like(p, memory_format=torch.preserve_format)
                    ...
                    # TODO : Correct for the zero-bias ? Not in paper
